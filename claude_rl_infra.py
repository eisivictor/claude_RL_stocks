...
class DQNAgent:
    ...
    def _build_model_new(self):
        ...
        # First 2 Conv layers
        # Need to reshape for Conv layers after LSTM
        model.add(Reshape((8, 8)))  # Reshape to proper dimensions for Conv1D - adjust as needed
        ...
        # Second 2 Conv layers - need to reshape again
        model.add(Reshape((8, 8)))  # Adjust dimensions as needed
        model.add(Conv1D(32, kernel_size=3, padding='same', activation='relu'))
        model.add(Conv1D(32, kernel_size=3, padding='same', activation='relu'))
        model.add(Flatten())
...
def train_agent(env, agent, episodes, batch_size, update_target_every=10):
    scores = []
    ...
    for e in range(episodes):
        ...
        print(f"START Episode: {e+1}/{episodes}, " + f"Epsilon: {agent.epsilon:.4f}")
        ...
        while not done:
            ...
            state = next_state
            total_reward += reward
            #print(f"VICTOR before train Episode: {e+1}/{episodes}")
            # Train the agent
            agent.replay(batch_size)
            #print(f"VICTOR after train Episode: {e+1}/{episodes}")
        ...
        print(f"Episode: {e+1}/{episodes}, Net Worth: ${info['net_worth']:.2f}, " +
...
def test_agent(env, agent):
    ...
    while not done:
        ...
        transactions.append(info['transactions'])
    ...
    print(f"Starting Balance: ${initial_value:.2f}")
...
def plot_results(name, df, balance_history, net_worth_history, transactions):
    ...
    for transaction in transactions[0]:  # Note: accessing first element since transactions is a nested list
